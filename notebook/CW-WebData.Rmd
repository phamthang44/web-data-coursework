...
head(summary_total_users_device, 6)
...
summary_top6_total_users <- summary_new_users_by_total %>%
  group_by(Device) %>%
  summarise(TotalNewUsers = sum(TotalNewUsers, na.rm = TRUE)) %>%
...
# Vẽ biểu đồ
ggplot(summary_top6_total_users, aes(x = Device, y = TotalNewUsers, fill = Device)) +
  geom_col() +
  geom_text(aes(label = TotalNewUsers), vjust = -0.3, size = 3) +
  labs(title = "New Users by Device",
